...
export const createCourse = async (req, res) => {
  try {
    ...
    if (type === 'Physical' && (!location || !String(location).trim())) {
      return res.status(400).json({ message: 'Location is required for physical courses' });
    }
    if (!link || !String(link).trim()) {
      return res.status(400).json({ message: 'Course link is required' });
    }
    if (!isValidUrl(link.trim())) {
      return res.status(400).json({ message: 'Course link must be a valid http(s) URL' });
    }
...
export const updateCourse = async (req, res) => {
  try {
    ...
    if (link !== undefined) {
      if (!String(link).trim()) return res.status(400).json({ message: 'Course link is required' });
      if (!isValidUrl(link.trim())) {
        return res.status(400).json({ message: 'Course link must be a valid http(s) URL' });
      }
